...
def validate_model(X, y, params):
    model = LGBMClassifier(**params, random_state=random_state, n_jobs=-1, verbosity=-1)
    return cross_validate_model(X, y, model, n_splits, random_state)


def fit_best_model(X, y, params):
    return LGBMClassifier(**params, random_state=random_state, n_jobs=-1, verbosity=-1).fit(X, y)
